...
// Function to get the name of the token from a contract using getNameToken
async function callGetNameToken(req) {
    ...
    try {
        ...
        // Call the getNameToken function using the contract address
        const result = await userContract.methods.getNameToken(contractToken).call();
        ...
        console.log("Error during getNameToken call:", error);
...
// Route handler for the getNameToken API
initializer.getNameToken = async function (req, res) {
    ...
    try {
        ...
        const response = await callGetNameToken(req);

        // Return the response (Success or Error)
        res.send(response);
        
    } catch (error) {
        console.log("Error in getNameToken route:", error);
